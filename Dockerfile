...
# Add a build argument to invalidate cache for the clone step
ARG CACHEBUST=1
...
# We explicitly pass --no-sandbox to ensure it runs correctly as root in Docker
# The '--' before --no-sandbox passes the argument to the underlying electron executable
CMD ["npm", "run", "webui:prod:remote", "--", "--no-sandbox"]
